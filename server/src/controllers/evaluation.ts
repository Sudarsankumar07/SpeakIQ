...
import { evaluateAudio } from '../services/gemini';
...
export async function createEvaluation(req: RateLimitRequest, res: Response): Promise<void> {
  ...
  if (!audio || !topic) {
    res.status(400).json({ error: 'Topic and raw voice recording are required' });
    return;
  }

  try {
    // Run multimodal Gemini audio evaluation
    const result = await evaluateAudio(audio, topic, chosenModel, fallbackTriggered);
    ...
    const dbResult = await pool.query(
      ...
      [
        userId,
        topic,
        result.transcript, // Save the transcript generated by Gemini from the audio
        finalModelUsed,
        result.grammar,
        result.vocabulary,
        result.fluency,
        ...
        result.feedback,
        audio, // Stores raw base64 audio URI
      ]
    );
